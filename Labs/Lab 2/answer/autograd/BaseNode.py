...
class relu(Node):
    ...
    def cal(self, X):
        self.cache.append(X)
        return np.clip(X, 0, None) # max(0, X)

    def backcal(self, grad):
        return np.multiply(grad, self.cache[-1] > 0) # 如果 cache[-1] > 0，grad 与 1 相乘，否则，grad 与 0 相乘
...
class sigmoid(Node):
    ...
    def backcal(self, grad):
        x = self.cache[-1]
        return np.multiply(grad, np.multiply(x, 1 - x))
...
class CrossEntropyLoss(Node):
    ...
    def backcal(self, grad):
        ...
        ret = np.zeros_like(X)
        np.put_along_axis(ret, np.expand_dims(y, axis = -1), -1, axis = -1)
        return np.multiply(grad, ret) / (X + EPS) # 防止溢出

...
class Graph(List):
    ...
    def forward(self, X, debug=False, removelossnode: int = 0):
        ...
        正向传播
        @param X: n*d 输入样本
        @param debug: 用于debug, print输入和输出数据的shap
    ...
    def optimstep(self, lr, wd1, wd2):
        ...
        """  
        for node in self:
            if node.num_params() > 0: # 需要先判断防止索引越界
                node.params[0] = node.params[0] - lr * (node.grad[0] + 2 * wd2 * node.params[0])
                node.params[1] = node.params[1] - lr * node.grad[1]

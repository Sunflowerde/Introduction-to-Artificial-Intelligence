    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            for action in legalActions:
                ...
                nextAgentIndex = index_of_agent + 1

                if nextAgentIndex == state.getNumAgents():
                    nextDepth = depth - 1
                    nextValue, _ = maximizer(successor, nextDepth, 0)
                else:
                    nextValue, _ = minimizer(successor, depth, nextAgentIndex)

                if nextValue > value:
                    value, maxiAction = nextValue, action
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            for action in legalActions:
                ...
                if nextValue < value:
                    value, miniAction = nextValue, action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in legalActions:
                successor = state.generateSuccessor(index_of_agent, action)
                agentIndex = index_of_agent + 1

                if agentIndex == state.getNumAgents():
                    nextDepth = depth - 1
                    nextValue, _ = maximizer(successor, nextDepth, 0, alpha, beta)    
                else:
                    nextValue, _ = minimizer(successor, depth, agentIndex, alpha, beta)
